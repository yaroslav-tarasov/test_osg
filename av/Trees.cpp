...
// copy constructor
Trees::Trees( const Trees & rhs, const osg::CopyOp & copyop )
...
void Trees::setWindFactor(float wf)
{
	_windFactor = wf;
	unfWindFactor_->set(wf);
}

void Trees::setGrassMapFactor(float val)
{
	unfGrassMapFactor_->set(val);
}

osg::Node*  Trees::_create()
{
	_geodeGrass = new osg::Geode();	
...
void   Trees::_buildStateSet(osg::Node* node)
{
...
osg::Geometry* Trees::_createGeometry()
{
