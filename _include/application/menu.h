...
struct menu : widget
{
    ...
    virtual size_t   add_string(std::string const &text, target const &click = target(), target const &hover = target()) = 0 ;
    virtual menu_ptr add_pop_up(std::string const &text) = 0 ;
