...
//        tow_rod_shape_ = bt_collision_shape_ptr(new btCylinderShape(to_bullet_vector3(point_3(0.1, half_dist, 0.1))));
        ...
        double const rod_mass = 1;    //cg::clamp(1000.0,650000.0,1.0,200.0)(tow_mass_); //1; //100; // TYV  на самый легкий и самый т€желый самолет (јн-225)
        double const rod_radius = .9; // 0.1;
        ...
        tow_constraint_self_.get()->setAngularLowerLimit(btVector3(-SIMD_HALF_PI, -SIMD_HALF_PI, -SIMD_HALF_PI/*btScalar(-80*cg::grad2rad())*/));
        tow_constraint_self_.get()->setAngularUpperLimit(btVector3(SIMD_HALF_PI, SIMD_HALF_PI, SIMD_HALF_PI/*btScalar(80*cg::grad2rad())*/));
