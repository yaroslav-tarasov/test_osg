...
#include "aircraft_fms_ext_model.h"
...
AUTO_REG_NAME(aircraft_fms_external_model, ext_model::create);

ext_model::ext_model( kernel::object_create_t const& oc, dict_copt dict )
    : view( oc, dict )
    , was_activated_(false)
{
    msg_disp()
        .add<msg::transition_msg>(boost::bind(&ext_model::on_transition, this, _1))
        .add<msg::fuel_discharge_msg>(boost::bind(&ext_model::on_fuel_discharge, this, _1))
        ;
}

void ext_model::on_object_created( object_info_ptr object )
{
...
void ext_model::reset_pos(geo_point_3 const& pos, double c)
{
    if (pilot_impl_)
    {
...
geo_point_3 ext_model::prediction(double dt) const
{
    if (pilot_impl_)
...
void ext_model::on_state(msg::state_msg const& msg)
{
...
void ext_model::on_controls(fms::manual_controls_t const& ctrl)
{
    if (pilot_impl_)
...
void ext_model::on_transition(msg::transition_msg const& t)
{
...
void ext_model::on_fuel_discharge(double part)
{
    if (pilot_impl_)
    {
...
void ext_model::on_assigned_fpl_changed()
{
    view::on_assigned_fpl_changed();
    init_fms();
}

void ext_model::on_procedure_changed()
{
    view::on_procedure_changed();
    init_fms();
}

void ext_model::on_fms_settings_changed()
{
