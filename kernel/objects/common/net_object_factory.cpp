...
object_info_ptr create_object( kernel::system* csys, create_msg const& msg)
{
    if(msg.object_kind & ok_vehicle)
        ...
        return create_aircraft_phl(csys, msg);  //create_aircraft(csys, msg); //  FIXME вместо чекера можно создать какой-нибудь более дурной объект
...
kernel::obj_create_data pack_object( kernel::system* csys, create_msg const& msg)
{
    if(msg.object_kind & ok_vehicle)
        ...
        return pack_aircraft_phl(csys, msg);  //pack_aircraft(csys, msg); //  FIXME вместо чекера можно создать какой-нибудь более дурной объект
