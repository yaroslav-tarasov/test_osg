...
visual::~visual()
{
    if (PK_NONE != settings_.kind)
        sys_->scene()->get_env_weather()->remove_local_bank(object_id()) ;
}
...
void visual::on_settings_changed()
{
    ...
    victory::environment_weather::local_bank_data bd;
    ...
    bd.heading         = 0.f ;
    bd.type            = victory::weather_params::precipitation_type(settings_.kind - 1) ;
    bd.intensity       = 1. ;
    bd.central_portion = 0.6f ;

    sys_->scene()->get_env_weather()->update_local_bank(object_id(), bd);
}
