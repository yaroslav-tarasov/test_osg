...
int main_shadows(int argc, char *argv[])
{
    ...
    */
    ...
    box->addChild(osgDB::readNodeFile("./shad_res/box.osgt"));
    box->setMatrix(osg::Matrix::translate(10, 2, -45));
    root->addChild(box.get());
    std::string vert = readShaderCode("./shad_res/box.vert");
    std::string frag = readShaderCode("./shad_res/box.frag");
    if (vert.length() && frag.length())
    {
        ...
        osgDB::writeNodeFile(*box->getChild(0), "./shad_res/box.osgx");
    }
