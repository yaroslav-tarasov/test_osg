...
inline void create_auto_object(kernel::system_ptr sys, std::string class_name, std::string unique_name)
{
    ...
    std::vector<object_class_ptr> const &classes = kernel::objects_factory_ptr(sys)->object_classes() ;
    ...
    auto obj = kernel::objects_factory_ptr(sys)->create_object(class_ptr, unique_name); 
