        ...
        for ( size_t j = 0; j < ima.getCount(); j++)
        {
            ...
            if (symbol.compare(materialName) == 0)
            {
                ...
                if (mat)
                {
                    ...
                    // TYV  save not used texcoords arrays for f.. future
                    if(textureUnit==0)
                    {
                        const osg::Geometry::ArrayList& al = cachedGeometry->getTexCoordArrayList();
                        for(int i = 0; i < al.size(); ++i )
                        {
                            clonedGeometry->setTexCoordArray(i, const_cast<osg::Array*>(al[i].get()));
                        }
                    }
                }
                else
